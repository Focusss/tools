...
public class PDFUtil extends PdfPageEventHelper {
	...
	public static String createPDF(String directoryPath,String fileName,JSONObject data)throws Exception {
		...
		// 创建输出流
		PdfWriter writer = PdfWriter.getInstance(doc, new FileOutputStream(new File(outPath)));
		// 设置页码，必须在doc.open()之前调用
		...
		Color color = new Color(228,242,253,1); //标题填充颜色
		
		CustomCellBottom cellBottom = new CustomCellBottom();
		
		PdfPCell cell;
		/*******基本信息*****/
		cell = new PdfPCell(new Phrase("基本信息", boldFont)); //用作标题
		cell.setBorder(Rectangle.NO_BORDER); //去除边框
		cell.setCellEvent(cellBottom); //增加底部虚线
		cell.setBackgroundColor(color); //填充标题颜色
	...
	/*public static void main(String[] args) throws Exception {
		// imageWaterMark(createPDF(), "C:\\Users\\cjwei7\\Desktop\\add.png");
		addWatermark(createPDF(), "广电银通");
	}*/
	...
	public static String createPDF() throws Exception {
		...
		// 输出路径
		String outPath = "C:\\Users\\cjwei7\\Desktop\\test.pdf";// .createTempPath(".pdf");
		...
		// 文本图片
		Image hand = Image.getInstance("C:\\Users\\cjwei7\\Desktop\\add.png");
		...
		writer.setPageEvent(new PdfPageFooter());
		/*
		 * 实现简单，但无法显示总页数，故采用监听方式，自定义实现 Font footerFont = new Font(bfChinese, 10,
		 * Font.NORMAL); HeaderFooter footer = new HeaderFooter(new
		 * Phrase("第",footerFont), new Phrase("页",footerFont));
		 * footer.setBorder(Rectangle.NO_BORDER);
		 * footer.setAlignment(Rectangle.ALIGN_CENTER); doc.setFooter(footer);
		 */
		...
		c6.setSkew(0, 30);
		Chunk c7 = new Chunk(
				" 出具，依据截止报告时间小微企业信用信息数据库记录的信息生成。除异议标注和查询记录外，报告中的信息均由相关报数机构和信息主体提供，",
				textFont);
		Chunk c8 = new Chunk("不保证其真实性和准确性，但承诺在信息整合、汇总、展示的全过程中保持客观、中立的地位。",
				textFont);
		ph1.add(c5);
		ph1.add(c6);
		ph1.add(c7);
		...
		p1 = new Paragraph("信息主体实缴资本**万元，****年**月**日股东****公司向***公司转让**万元股权。",
				textFont);
		...
		p1 = new Paragraph("信息主体最近一次缴纳社会保险为****年**月**日，最近一次缴纳公积金为****年**月**日。",
				textFont);
		p1.setFirstLineIndent(23);
		doc.add(p1);
		...
		table = createCell(table, new String[] { "出资方名称", "证件类型", "证件号码", "币种",
				"出资金额", "出资方式", "出资占比", "数据来源", "采集时间" }, 2, 9);
		doc.add(table);
		...
		table.setTotalWidth(new float[] { 55, 55, 55, 55, 55, 55, 55, 55, 55,
				55 }); // 设置列宽
		table.setLockedWidth(true); // 锁定列宽

		table = createCell(table, new String[] { "处罚文书号", "裁定处罚部门", "处罚时间",
				"违法或违规行为描述", "涉及金额", "处罚金额", "处罚决定", "整改情况", "数据来源", "采集时间" },
				2, 10);
		doc.add(table);
		...
		table = createCell(table, new String[] { "案号", "立案法院", "立案日期", "案由",
				"执行标的", "执行标的金额", "已执行标的" }, 2, 7);
		table = createCell(table, new String[] { "已执行标的金额", "执行标的日期", "案件状态",
				"结案日期", "执行结案方式", "数据来源", "采集时间" }, 2, 7);
		doc.add(table);
		...
		table = createCell(table, new String[] { "编号", "授信金融机构", "授信额度",
				"起始日期", "终止日期", "银行内部评级结果", "数据来源", "采集时间" }, 2, 8);
		doc.add(table);
		...
		table = new PdfPTable(10);
		table.setTotalWidth(new float[] { 30, 55, 75, 55, 55, 55, 55, 55, 55,
				55 }); // 设置列宽
		table.setLockedWidth(true); // 锁定列宽

		table = createCell(table, new String[] { " ", "编号", "贷款金融机构", "贷款余额",
				"贷款日期", "到期日", "五级分类", "欠息金额", "数据来源", "采集时间" }, 1, 10);
		...
		table = createCell(table, new String[] { "编号", "贴现金融机构", "贴现金额", "贴现日",
				"贴现率", "五级分类", "数据来源", "采集时间" }, 2, 8);
		doc.add(table);
		...
		table = createCell(table,
				new String[] { "序号", "查询日期", "查询用户", "查询原因" }, 3, 4);
		doc.add(table);
		...
		p1.setSpacingAfter(10);
		ph1 = new Phrase();
		Chunk c89 = new Chunk("异议标注（用于记录信息主体对某类信息提出异议处理情况，置于每类信息项下方）：",
				redBoldFont);
		Chunk c90 = new Chunk(
	 ...
	 */
	private static PdfPTable createCell(PdfPTable table, String[] title,
			int row, int cols) throws DocumentException, IOException {
		// 添加中文字体
		BaseFont bfChinese = BaseFont.createFont("STSong-Light",
				"UniGB-UCS2-H", BaseFont.NOT_EMBEDDED);
	...
	public static String addWatermark(String inputFile, String waterMarkName)
			throws Exception {
		String[] spe = separatePath(inputFile);
		String outputFile = spe[0] + "_WM." + spe[1];

		PdfReader reader = new PdfReader(inputFile);
		PdfStamper pdfStamper = new PdfStamper(reader, new FileOutputStream(
				outputFile));
	...
	static class CustomCellBottom implements PdfPCellEvent {
		public void cellLayout(PdfPCell cell, Rectangle position,PdfContentByte[] canvases) {
		    ...
		    cb.moveTo(position.getLeft(), position.getBottom()); //从哪个位置开始画，左下角
		    cb.lineTo(position.getRight(),position.getBottom()); //画到哪里停止，右下角
		    cb.stroke();
		    cb.restoreState();
